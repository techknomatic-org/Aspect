...
export const ReportsPage: React.FC = () => {
  ...
  const card = isLight ? 'bg-white border-slate-200 shadow-sm hover:border-gold/60' : 'bg-[#0E172E]/90 border-navy-700/60 shadow-xl hover:border-gold/40';
  const textPrimary = isLight ? 'text-slate-900' : 'text-white';
  const textSec = isLight ? 'text-slate-500' : 'text-slate-400';
  const dlBtn = isLight ? 'bg-slate-100 border-slate-300 text-gold hover:bg-gold hover:text-white' : 'bg-navy-800 border-slate-700 text-gold hover:bg-gold hover:text-navy-950';
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs ${textSec}`}>Encrypted Governance Archive • ISO 27001 Certified</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {reports.map((rep, idx) => (
          <div
            key={idx}
            className={`p-5 rounded-2xl border flex items-center justify-between transition-all duration-200 cursor-pointer group ${card}`}
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className={`text-sm font-extrabold mt-0.5 group-hover:text-gold transition-colors ${textPrimary}`}>{rep.title}</h3>
                <span className={`text-[11px] block mt-1 ${textSec}`}>{rep.period} • {rep.size} • Published {rep.date}</span>
              </div>
            </div>

            <button
              onClick={() => alert(`Downloading ${rep.title}...`)}
              className={`p-2.5 rounded-xl border transition-colors shrink-0 ${dlBtn}`}
