...
export const RiskIntelligencePage: React.FC = () => {
  ...
  const riskCategories = [
    { title: 'CRITICAL RISKS',  count: 3,   color: 'text-rose-400',   bg: isLight ? 'bg-rose-50 border-rose-200'   : 'bg-rose-950/80 border-rose-500/50',   desc: 'Requires immediate CEO Steering Committee intervention' },
    { title: 'HIGH PRIORITY',   count: 12,  color: 'text-amber-500',  bg: isLight ? 'bg-amber-50 border-amber-200' : 'bg-amber-950/80 border-amber-500/50',  desc: 'SLA delay and operational variance tracking' },
    { title: 'MONITORING',      count: 113, color: 'text-slate-500',  bg: isLight ? 'bg-slate-100 border-slate-300' : 'bg-slate-800 border-slate-700',       desc: 'Routine telemetry & compliance audits' },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {riskCategories.map((cat, idx) => (
          <div key={idx} className={`p-5 rounded-2xl border ${cat.bg} flex items-center justify-between`}>
            <div>
              <span className={`text-[10px] font-extrabold tracking-widest uppercase ${textSec}`}>{cat.title}</span>
              <span className={`text-3xl font-outfit font-extrabold block mt-1 ${cat.color}`}>{cat.count} Issues</span>
              <p className={`text-[11px] mt-1 ${textSec}`}>{cat.desc}</p>
            </div>
      ...
      <div className={`${card} border rounded-2xl p-6`}>
        ...
        <div className="space-y-3 mt-4">
          {activeAlerts.map((alert) => (
            <div key={alert.id} className={`p-4 rounded-xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-xs ${innerCard}`}>
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-extrabold text-gold uppercase">{alert.division}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase ${
                    alert.status === 'Critical' ? 'bg-rose-950/60 text-rose-400 border border-rose-500/50' : 'bg-amber-950/60 text-amber-400 border border-amber-500/50'
                  }`}>
                    {alert.status}
                  </span>
                </div>
                <h4 className={`text-sm font-bold mt-1 ${textPrimary}`}>{alert.issue}</h4>
                <p className={`text-[11px] mt-0.5 ${textSec}`}><strong>Financial Exposure:</strong> <span className="text-rose-400 font-semibold">{alert.impact}</span></p>
              </div>

              <div className="text-right shrink-0">
                <span className={`text-[10px] block ${textSec}`}>Recommended Action</span>
